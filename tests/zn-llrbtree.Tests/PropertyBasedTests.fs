        ...
        property {
          ...
          Expect.isTrue
            (LLRBTree.forall (fun e ->
              e >= System.Int32.MinValue && e <= System.Int32.MaxValue) t)
            "forall"
          Expect.equal (LLRBTree.forall (fun e -> e >= 0) t)
            (LLRBTree.min t >= 0) "forall vs. min"
        }
        |> Property.check
      testCase "collect" <| fun _ ->
        property {
          ...
          Expect.equal (LLRBTree.toList t)
            (LLRBTree.collect LLRBTree.singleton t |> LLRBTree.toList)
            "collect"
